...
void sort_edge_set(edge *array,int left,int right){
	int i=left,j=right;
	edge tmp;
	...
	if(left<j)
	    sort_edge_set(array,left,j);
	if(i<right)
	    sort_edge_set(array,i,right);	
}
...
int main(int argc, char ** argv){
		...
		//for (i=0; i<no_of_partitions; i++){	sort_edge_set(edge_set[i], 0, no_of_edges[i]-1);}	
